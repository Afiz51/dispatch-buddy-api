...
const register = async (req, res) => {
  const { name, email, phoneNum, user_type, password, address, city } =
    req.body;
  ...
  if (user_type === "shipper") {
    ...
    if (result.error) {
      throw new BadRequestError(
        transformJoiMsg(result.error.details[0].message)
      );
    }
  }
  ...
  if (user_type === "rider") {
    ...
    await validateImageFile(
      bikeDocument.mimetype.startsWith("application"),
      "Invalid, please upload a valid bike document"
    );
    await validateImageFile(
      passport_photo.mimetype.startsWith("image"),
      "Invalid, please upload a valid passport photo"
    );
    await validateImageFile(
      valid_IdCard.mimetype.startsWith("image"),
      "Invalid, please upload a valid ID card"
    );

    const maxSize = 1024 * 1024;

    if (
      bikeDocument.size > maxSize ||
      passport_photo.size > maxSize ||
      valid_IdCard.size > maxSize
    ) {
